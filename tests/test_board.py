...
tests/test_board.py
===================
Estas son las "pruebas de calidad" del tablero.

Piensa en esto como un inspector de fábrica que revisa cada pieza
antes de que salga al mercado. Si estas pruebas pasan, el tablero está sano.

Para correrlas, escribe en la terminal:
...
def test_tablero_vacio_tiene_dimensiones_correctas():
    """Un tablero nuevo debe saber cuánto mide."""
...
def test_colocar_una_palabra_horizontal():
    ...
    assert tablero.esta_vacia(1, 4)  # La cuarta casilla sigue vacía
...
def test_dos_palabras_se_cruzan():
    ...
    # CRUCE REAL: "CASA" horizontal en (2,0): C-A-S-A
    # "SAL" vertical en (0,2): S-A-L → se cruzan en (2,2) con la letra 'S'
    p_casa = Placement("CASA", fila=2, columna=0, direccion=Horizontal())
    p_sal = Placement("SAL", fila=0, columna=2, direccion=Vertical())
    
    tablero.colocar(p_casa)
    tablero.colocar(p_sal)
    
    assert tablero.celda(2, 2) == "S"  # Ambas palabras ponen 'S' aquí
    assert len(tablero.placements) == 2


def test_quitar_palabra_libera_celdas():
    """Si quito una palabra, sus casillas deben volver a estar vacías."""
    tablero = Board(filas=5, columnas=5)
    p = Placement("SOL", fila=1, columna=1, direccion=Horizontal())
    tablero.colocar(p)
    tablero.quitar(p)
...
def test_quitar_palabra_no_borra_cruce():
    """Si dos palabras comparten una casilla y quito una, la otra debe seguir ahí."""
    tablero = Board(filas=5, columnas=5)
    p1 = Placement("CASA", fila=2, columna=0, direccion=Horizontal())
    p2 = Placement("SAL", fila=0, columna=2, direccion=Vertical())
    
    tablero.colocar(p1)
    tablero.colocar(p2)
    tablero.quitar(p1)  # Quitamos CASA, pero SAL sigue
    
    assert tablero.celda(2, 2) == "S"  # SAL todavía pone 'S' aquí
    assert tablero.esta_vacia(2, 0)    # La 'C' de CASA desapareció
    assert tablero.esta_vacia(2, 1)    # La 'A' de CASA desapareció
    assert tablero.esta_vacia(2, 3)    # La 'A' de CASA desapareció
